...
def load_random_connected_component_pa(path, min_size=500):
...
def scheme_random_components_pa(path, q, times, min_size=500):
    removed = []
    for _ in tqdm(range(times)):
        g, r = load_random_connected_component_pa(path, min_size)
...
qs = [0.2, 0.4, 0.6]
for q in qs:
    removed = scheme_random_components_pa("roadNet-PA.txt", q=q, times=100)
    avg, var = calc_avg_and_variance(removed)
    print(f"Random components (roadNet-PA) with q={q}: Avg removed ratio = {avg:.3f}, Var = {var:.5f}")
